    ...
    useEffect(() => {
        ...
        if (Array.isArray(displayTimes) && displayTimes.length !== 0){
            setFilteredClips(displayTimes.map((clip) => {
                return {id: clipCounter++, start: clip[0], end: clip[1]}
            }))
            setNoClips(false)
          ...
          {!proceed ? (
            <>
             {!noClips ? (
              ...
              <p class='mt-5 offset-3 ps-4 col-6'><b>*Note: </b>Found clips are ordered by descending "heat", i.e. most rewatched clips come first</p>
              ...
              {displayTimes === -1 ? (
                ...
                {Array.isArray(displayTimes) && displayTimes.length !== 0 && (
                  <div class='row restore-row'>
                    <button class='restore text-center' onClick={handleRestore}>Restore Original Clips</button>
                  </div>
                )}<br/>
                </div>
                <button className="back2" title="Back" onClick = {(e) => handleBack(e)}><img src="/back.svg" alt="Back button"/></button>
...
const Clip = props => {
    return (
        <div className='clips'>
            ...
            <div className="edits row">
              <button class='delete col-2' onClick={props.removeClip}>Delete Clip</button>
              <EditTimesForm 
...
const EditTimesForm = props => {
    ...
    return (
      <>
        <form class='col-8 text-center' id="clip-times">
            <label>
                Clip Start:&nbsp;
        ...
        <button class="edit-times col-2" form="clip-times" type="submit" onClick={(e) => onSubmit(e)}>Edit Times</button>
      </>
    )
  }
